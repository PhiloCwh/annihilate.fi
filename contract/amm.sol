...
//全局变量
    ...
    uint constant ONE_YEAR = 86400 * 365;
...
//borrow logic
    ...
    function stakeETHAndBorrow(address _token)public payable {
       ...
       // address addr = address(this);
        ...
        borrowAmount = calSwapTokenOutAmount(WETHAddr,_token , ETHAmount) * 50 / 100;
    ...
    function calStakeETHAndBorrow(address _token,uint ETHAmount)public view returns(uint borrowAmount) {
    ...
    function closeShortPosition(address _token,uint _amount) public {
        ...
        if(swapTokenAmount <= findUserBorrowedAmount[msg.sender][lpAddr][_token]){
            ...
            swap(WETHAddr, _token,swapAllDebtWethAmountIn);
            redeemETH(_token, swapAllDebtWethAmountIn);
    ...
    function calShortTokenSwapAmount(address _token,uint _amount)public view returns(uint){
        ...
        if(swapTokenAmount > findUserBorrowedAmount[msg.sender][lpAddr][_token]){
            ...
            return calRedeemETHAmount(_token, swapTokenAmount);
        }
    ...
    function calHelthyFactor(address _user,address _token)public view returns(uint userWethLpDebt,uint userWethLpBalance ,uint helthyFactor){
        address lpAddr;
        //uint userLpAsset;
        uint userLpDebt;
        //uint helthyFactor;
       ...
       //lpAsset = findUserBorrowedAmount[_user][lpAddr][_token];
        ...
        //70% liquidate
        userWethLpBalance = userLpETHBalance[_user][lpAddr];

        helthyFactor = ONE_ETH * userWethLpDebt/userWethLpBalance;
    }
    ...
    function liquidateWethLp(address _user,address _token) public {
        ...
        helthyFactor = ONE_ETH * lpWethDebt/lpAsset;

        if(helthyFactor >= 7*10**17){

            //AMM.swap(WETHAddr, _token, lpAsset);


            if(lpWethDebt < lpAsset){

                uint profit;
                profit = lpAsset - lpWethDebt;
    ...
    //repay logic
    function repayToken(address _tokenA,address _tokenB,address _token,uint _amount) public {
        ...
        //uint tokenReserve = reserve[lpAddr][_token];
        address user = msg.sender;
        require(_amount < findUserBorrowedAmount[user][lpAddr][_token],"require smaller than loun");
        IERC20 token = IERC20(_token);
        ...
        findUserBorrowedAmount[user][lpAddr][_token] -= _amount;
        //findBorrowedAmount[lpAddr][_token] -= _amount;
        findAllUserDebt[lpAddr][_token] -= _amount;
